...
WORKLOADS = [
    {
        "key": "emitted_messaging",
        "label": "Portable native messaging service",
        "mira_source": ROOT / "mira" / "examples" / "runtime_emitted_messaging_service.mira",
        "mira_function": "emitted_messaging_request_batch",
        "iterations": 8,
        "unit_work": 8,
    },
    {
        "key": "emitted_analytics",
        "label": "Portable native analytics service",
        "mira_source": ROOT / "mira" / "examples" / "runtime_emitted_analytics_service.mira",
        "mira_function": "emitted_analytics_request_batch",
        "iterations": 8,
        "unit_work": 8,
    },
]
...
def render_markdown(matrix: dict) -> str:
    lines = [
        "# MIRA 2.5.0 Portable Native Supremacy Matrix",
        "",
        f"Selected dominant production backend: `{matrix['selected_default_backend']}`",
        "",
        "| Workload | Runtime | Median ns | p95 ns | RSS bytes | Classification vs `mira_c` |",
        "| --- | --- | ---: | ---: | ---: | --- |",
    ]
...
def main() -> int:
    ...
    for workload in WORKLOADS:
        for backend in ("mira_default", "mira_c", "mira_arm64", "mira_x86_64"):
            ...
            by_workload.setdefault(workload["key"], {})[backend] = row["median_ns"]

    wins_vs_c = 0
    classifications = {}
    for workload in WORKLOADS:
        runtimes = by_workload[workload["key"]]
        classification = classify(runtimes["mira_default"], runtimes["mira_c"])
        classifications[workload["key"]] = classification
        if runtimes["mira_default"] <= runtimes["mira_c"]:
            wins_vs_c += 1
    for row in rows:
        row["classification"] = (
            classifications[row["workload"]] if row["runtime"] == "mira_default" else ""
        )

    rationale = (
        "`mira_default` selected because the portable native emitted path stays ahead of "
        f"the legacy C-backed path on {wins_vs_c}/{len(WORKLOADS)} promoted messaging/analytics workloads, "
        "with at least one workload in the clear-ahead band and no promoted claim relying on compile-only evidence."
    )
    matrix = {
        "release": "2.5.0",
